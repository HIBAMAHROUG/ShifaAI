...
ShifaaAI is a medical symptom analysis platform with a Flask backend and a React frontend. It accepts free-text symptom descriptions, predicts likely conditions, and enriches results with external medical sources when available.

## Project Structure

- `SHIFAAAI/backend/` - Flask API, controllers, services, and data files.
- `SHIFAAAI/frontend/` - React frontend shell and browser assets.
- `SHIFAAAI/database/` - Database schema and initialization scripts.
- `SHIFAAAI/ai-engine/` - Model training and evaluation helpers.

## Requirements

- Python 3.10 or newer
- `pip`
- Optional Infermedica credentials for the external diagnosis integration
...
To open the frontend locally, serve `SHIFAAAI/frontend/` with a static server and open `index.html`.
...
| POST | `/api/infermedica/parse` | Infermedica symptom parsing |

## Notes

- Legacy demo and test files were removed to keep the repository focused on production entrypoints.
- Active backend entrypoint: `SHIFAAAI/backend/app.py`.
- Active frontend entrypoint: `SHIFAAAI/frontend/index.html`.
